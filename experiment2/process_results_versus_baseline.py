...
JSON_FILEG = 'results/gray_baseline.json'
...
with open(JSON_FILEG) as fh:
    resultsg = json.load(fh)
g_found = resultsg[FOUND_KEY]
...
gray_found = {}
for item, color_dict in g_found.items():
    gray_found[item] = defaultdict(int)
    for col, amount in color_dict.items():
        if col in color_keys:
            color_name = col.split('_')[-1]
            gray_found[item][color_name] += amount
...
for item, total_dict in stripped_total.items():
    ...
    gray_recall_dict[item] = defaultdict(int)
    found_dict = stripped_found[item]
    gray_found_dict = gray_found[item]
    for col, total_amount in total_dict.items():
        if total_amount >= THRESHOLD:
            found_amount = found_dict[col]
            gray_found_amount = gray_found_dict[col]
            ...
            if gray_found_amount == 0:
                recall = 0
            else:
                recall = gray_found_amount / total_amount
            gray_recall_dict[item][col] = (recall, total_amount)
...
recall_per_cat_gray = defaultdict(list)

for item, color_recall_dict in gray_recall_dict.items():
    ...
    if len(sorted_dict) > MIN_NUM:
        ...
        for key, val in sorted_dict.items():
            ...
            recall_per_cat_gray[key].append(val[0])#
...
for item, color_recall_dict in recall_dict.items():
    ...
    if len(sorted_dict) > MIN_NUM:
        ...
        for key, val in sorted_dict.items():
            print(key, val[0], val[1])
            recall_per_cat[key].append(val[0])# / sum([i[0] for i in sorted_dict.values()]))


print('\nTotal\n')
final_recall_dict = {}
for k, v in recall_per_cat.items():
    gv = recall_per_cat_gray[k]
    final_recall_dict[k] =  (sum(v)/ len(v)) - (sum(gv) / len(gv)) 
...
for obj, new_dict in recall_dict.items():
    gdict = gray_recall_dict[obj]
    if len(new_dict) > MIN_NUM:
        print(obj, '&', ' & '.join('-' if k not in new_dict else str(round(new_dict[k][0] - gdict[k][0], 3 )).replace('0.', '.') for k in keys))
